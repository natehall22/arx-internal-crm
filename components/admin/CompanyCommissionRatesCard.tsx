...
/**
 * Admin editor for the org-wide derived commission rates that payroll reads off the
 * org — inspection and self-generated — via `GET/POST /api/admin/comp-rates`.
 *
 * The manager override is deliberately NOT edited here. Since the management-overlay
 * work (migration 202608070001) payroll resolves override lines from effective-dated
 * overlay assignments and plan versions, and `buildAdditiveParticipantsForJob` never
 * reads `orgs.manager_override_commission_rate`. An input for it on this card would
 * be a control that silently pays nothing, so the override lives in its own section
 * on /admin/comp-plans instead.
 *
...
export default function CompanyCommissionRatesCard() {
  ...
  const save = async (overrides?: { confirm_disable?: boolean; confirm_backdate?: boolean; apply_to_later_rows?: boolean }) => {
    ...
    if (![inspection, selfGen].every((n) => Number.isFinite(n))) {
      setError('Enter a number for both rates')
      return
    }
    if (!effectiveFrom) {
      ...
      if (!res.ok) {
        ...
        if (data.code === 'later_rows_shadow_warning') {
          const laterRowsText = (data.laterRows || [])
            .map(
              (row) =>
                `  ${row.effectiveFrom}: inspection ${row.inspectionRate}%, self-gen ${row.selfGenRate}%`
            )
            .join('\n')
  ...
  return (
    <section className="mb-8 bg-white rounded-xl shadow-sm border p-6 sm:p-8">
      ...
      <p className="mt-2 max-w-3xl text-sm" style={{ color: INK }}>
        The org-wide derived pay lines added on top of the base commission: inspection and
        self-generated. Each is a percent of a job&apos;s commissionable base and applies only
        when no explicit per-job override exists. A rate of 0 disables that line entirely.
        Changes are effective-dated — the resolver always uses the latest row on or before a
        job&apos;s sale date, so a later scheduled row can shadow one you just saved (see the
        warning below if that applies).
      </p>
      <p className="mt-2 max-w-3xl text-sm" style={{ color: INK }}>
        <strong>The manager override is not set here.</strong> It is paid per manager and per
        production lane from an effective-dated overlay assignment — set it in{' '}
        <span className="font-semibold">Manager override</span> on the User Assignments tab
        below.
      </p>
        ...
        <>
          ...
          <div className="mt-4">
            ...
            <textarea
              ...
              style={{ color: INK }}
              placeholder="e.g. Enabling the self-generated line per the 2026 comp ladder"
            />
          </div>
            ...
            <div className="mt-2 overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  ...
                  {history.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-3 text-sm" style={{ color: INK }}>
                        No history yet.
                      </td>
                    </tr>
                  )}
