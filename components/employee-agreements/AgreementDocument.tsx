...
export default function AgreementDocument({ agreement, employeeName, effectiveDate }: { agreement: EmployeeAgreementTemplate; employeeName: string; effectiveDate: string }) {
  return <article className="rounded-xl border bg-white p-4 sm:p-6 text-sm leading-6 text-gray-800 shadow-inner">
    ...
    <dl className="mb-6 grid grid-cols-1 sm:grid-cols-[130px_1fr] gap-x-3 gap-y-1 rounded-lg bg-gray-50 p-4"><dt className="font-semibold">Team member</dt><dd>{employeeName}</dd><dt className="font-semibold">Effective date</dt><dd>{effectiveDate}</dd><dt className="font-semibold">Agreement version</dt><dd>{agreement.version}</dd></dl>
