...
- **Cancelled jobs (added 2026-09-14).** A sold job that won't be installed is `production_jobs.status = 'cancelled'`, written ONLY by `POST /api/ops/jobs/[id]/cancel` → `cancel_production_job` (atomic: job + project → cancelled, sale agreement → `voided`, job note). A reason code is required (`JOB_CANCELLATION_REASONS` in `lib/job-status.ts`; `other` needs notes) so cancels can be tracked and credit fails worked again. **The opportunity is NOT touched** (Nathan's call). Sales counts drop the deal because they all filter contracts on `status = 'completed'`; any **job-based** query that means "sales" or "ops work" and doesn't allowlist statuses must `.neq('status', CANCELLED_JOB_STATUS)`. Re-signing reinstates the job dated today (`reinstateCancelledJob`); an admin/owner can undo a mistaken cancel (`DELETE` on the same route → `uncancel_production_job`, keeps the original sale date).
  **PERMISSION REVIEW DUE 2026-11-14:** anyone with ops `jobs:edit` can cancel, "for now". If ARX has ops management by then, that becomes the floor — change `canEditJobs` in the cancel route.
...
| 9 | **"What was sold on this job" — down to one resolver left.** 2026-09-05: the squares + roof-measure chain was collapsed into `lib/job-sold-scope.ts` (`buildJobSoldScope`, `resolveJobProposalId`, `resolveJobOpportunityId`, `findJobRoofMeasurementRow`), now the single source for `/ops/jobs/[id]`, the materials-order print sheet, and `lib/job-run-sheet.ts`. Two paths were deliberately left alone: (a) `components/ops/SoldScopeCard.tsx` re-fetches the proposal + line items **client-side** through the browser/anon client with its own `linked ?? accepted` precedence and its own with-squares/without-squares fallback select; (b) `app/ops/jobs/[id]/page.tsx` ~L147-210 resolves the job's opportunity from `order_form_contracts` → address match → `project.opportunity_id`, which `resolveJobOpportunityId` does not fully reproduce — the page passes its answer in as an override. **Partially closed 2026-09-05:** `resolveJobOpportunityId` gained an org-scoped `address_text` leg that runs only after proposal → project both come up empty, because without it the supplier order sheet resolved a *different* opportunity than the card ops read on screen and printed a blank sheet for jobs whose measurement is linked by `opportunity_id` only. It still does not reproduce the page's `order_form_contracts` leg. **(a) closed 2026-09-06:** `SoldScopeCard` no longer fetches proposals/`proposal_line_items` client-side or runs its own `linked ?? accepted` precedence / squares resolution (`lib/sold-roof-squares.ts`) — it now takes a required `scope: JobSoldScope | null` prop (the job page passes `job.sold_scope`, the same `buildJobSoldScope` output every other consumer uses) and renders squares, the proposal link/number, and line items straight from it. It keeps one narrow, ID-anchored client fetch — `proposals` by `scope.proposal_id` — purely for cosmetic fields `JobSoldScope` doesn't carry ($ total, free-text scope, accepted date) for the AI job packet; that fetch can't disagree with the rest of the page since the id is already fixed by `scope`. Also found and deleted the same day: a third, undocumented resolver, `components/ProjectSoldScope.tsx` on `/projects/[id]`, keyed only on `project_id` with no linked-proposal/opportunity fallback at all — replaced with a link to the job's Sold Scope card on `/ops/jobs/[id]` when a production job exists for the project, or the project's raw `sold_roof_squares` field (no line items, no resolution) when it doesn't. (b) is still open. | 1 path | Medium — (b) means "which opportunity backs this job" still has two answers. Not safe to fold in as a drive-by: it changes which measurement the job page picks. |
...
| 11 | **A job's cost is computed FIVE different ways, and they disagree.** Two write paths — "+ Job Cost" on the Materials tab (`job_product_orders`) and "+ Add cost line" under Photos & files (`job_cost_lines`) — with nothing telling ops they are different systems. Five readers: the job page Financials card (`JobDetailClient.tsx` ~L1206-1228, reads `job_product_orders` live, **never reads `job_cost_lines`**), the ops board `getProfitability()` (`OpsClient.tsx` ~L210-228, reads the stored `production_jobs.material_cost` column, which a 2026-07-08 bulk spreadsheet import overwrote out-of-band, so it can be stale), `/admin/job-profit-tracker` (~L316-347, sums `misc`/`other`/`permit` cost lines unconditionally **on top of** the material column), `lib/goals-job-economics.ts` (same pattern), and `lib/payroll-period-materialization.ts`. Audited against prod 2026-09-07. | 2 write paths, 5 readers | **High — real money, verified in prod.** Omission: 26-0012/0022/0018/0010 carry $1,774.70 of cost lines the job page profit ignores. Double-count: **26-0041** has the same $5,421.03 "Interior Work" in both tables (entered a day apart by someone who noticed it wasn't showing) and **26-0013** has the same $918 lending fee as two separate lines 35s apart — the profit tracker counts each twice. **Payroll landmine:** any unapproved `job_cost_lines` row with `deduct_from_commission_base=true` (both are the default) **blocks that job from payroll materialization entirely** (`derivePayrollEligibility`, `missing_costs`); 26-0041 carries one now and will stall on completion. Note `deductible_costs_total` is computed and written to `payroll_job_snapshots` but **read by nothing** — the amount never actually reduces a payout today. Interim shipped 2026-09-07: the Financials card now shows a labelled "Cost lines (Photos & files)" row stating it is NOT in Est. profit, rather than silently omitting it or naively adding it (which would double-count 26-0041). The real fix is consolidating onto `job_cost_lines` (it has cost_type/approval/attachments; `job_product_orders` has none) — a data migration needing a dry run and Nathan's + Andrew's sign-off. **Do NOT wire `deductible_costs_total` into the comp base as part of that; it changes what people get paid and needs its own decision.** |

| 12 | **Payroll still computes monthly sales volume, but volume tiers are gone.** The live comp plan (`~/dev/arx-website/src/lib/comp-plan.ts`, arxroofing.com/join) has no volume bonus, yet `buildMonthlyVolumeMaps` in `lib/payroll-export.ts` still runs in `app/api/admin/payroll/export/route.ts` and `lib/payroll-period-materialization.ts`. Confirmed with Nathan 2026-09-14. Also found the same day: `app/api/contracts/sign/route.ts` carries two near-identical "create or update the production job" blocks (new-project ~L620 and existing-project ~L825). | 2 volume readers; 2 job-create copies | Low — dead payroll math is misleading, not wrong-paying. Delete the volume path after confirming nothing still reads its output. Fold the sign-route copies together only with care: that file also carries the live signed-contract data-loss bug. |

**Rule going forward:** items 1, 3b, 4, 5 and 11 are the ones that can cause a real incident.
