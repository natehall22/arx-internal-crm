 ...
 * override, self-generated — with full effective-dated history.
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const rates: Record<RateKey, number> = {} as Record<RateKey, number>
    ...
    const disabling = (Object.keys(fields) as RateKey[]).filter(
      (key) => currentValues[key] > 0 && rates[key] === 0
    )
    ...
    const { error } = await supabase.rpc('upsert_org_derived_commission_rates', {
      p_org_id: orgId,
      p_inspection: rates.inspection_rate,
      p_manager_override: rates.manager_override_rate,
      p_self_gen: rates.self_gen_rate,
