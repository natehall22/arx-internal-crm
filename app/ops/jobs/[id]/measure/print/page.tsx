...
export default async function ExteriorMeasurePrintPage({ params }: { params: { id: string } }) {
  ...
  if (!measure.report) redirect(`/ops/jobs/${params.id}/measure`)
  ...
  const customer = Array.isArray(subject.customer) ? subject.customer[0] : subject.customer
  ...
  const customerName = customer?.name || lead?.homeowner_name || 'Customer'

  return (
    <main className="min-h-screen bg-white text-gray-950">
      <div className="no-print mx-auto flex max-w-5xl items-center justify-between px-4 py-4">
        <Link href={`/ops/jobs/${params.id}/measure`} className="text-sm font-medium text-indigo-600">
          Back to measure
        </Link>
        <PrintReportButton />
      </div>
        ...
        <header className="border-b-4 border-gray-950 pb-6">
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600">{customerName} · {subject.job_number || 'Opportunity measure'}</p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Wall {fmt(n(elevation.wall_width_ft))} ft × {fmt(n(elevation.wall_height_ft))} ft
                      {n(elevation.gable_width_ft) > 0 || n(elevation.gable_height_ft) > 0
                        ? ` · Gable ${fmt(n(elevation.gable_width_ft))} ft x ${fmt(n(elevation.gable_height_ft))} ft`
                        : ''}
                    </p>
                  </div>
                    ...
                    <tbody>
                      {elevation.openings.map((opening: any) => (
                        <tr key={opening.id} className="border-b">
                          ...
                          <td className="py-2 text-right">{fmt(n(opening.width_ft))} × {fmt(n(opening.height_ft))} ft</td>
