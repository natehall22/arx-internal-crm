...
export default function NewWorkOrderClient({
  ...
  const getInitialFormData = () => {
    ...
    let customer_id = initialCustomerId || ''
    let project_id = initialProjectId || ''

    if (initialProjectId) {
      const project = projects.find(p => p.id === initialProjectId)
      if (project) {
        customer_id = project.customer_id || customer_id
        const parsed = parseAddress(project.address_text || initialAddress || '')
      ...
      if (customer?.address_text) {
        const parsed = parseAddress(customer.address_text)
        address = parsed.address
        city = parsed.city
        state = parsed.state
        zip = parsed.zip
  ...
  const handleProjectChange = (projectId: string) => {
    ...
    if (project) {
      ...
      if (project.address_text) {
        const parsed = parseAddress(project.address_text)
        setFormData(prev => ({
          ...prev,
          ...parsed,
        }))
      ...
      if (formData.work_order_type === 'go_back') {
        ...
        if (opsUsers && opsUsers.length > 0) {
          const notifications = opsUsers
            .filter(u => u.id !== workOrderData.assigned_user_id)
            .map(u => ({
              org_id: orgId,
              user_id: u.id,
