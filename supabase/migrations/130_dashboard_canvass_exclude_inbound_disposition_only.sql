-- Door/contact dashboard aggregates: stop treating inbound web-style leads as “canvass doors”
-- when they only have a canvass_disposition set. Previously: (canvass source) OR (any disposition),
-- so CRM edits or bad data could mark web/inbound leads with a disposition and they counted as knocks.
-- Still counts: field sources, csv_import, and any non–web/inbound lead with a disposition.
...
AS $$
  ...
  WHERE l.org_id = p_org_id
    ...
    AND (
      lower(trim(COALESCE(l.source::text, ''))) IN ('door_to_door', 'canvass', 'door_knock', 'csv_import')
      OR (
        l.canvass_disposition IS NOT NULL
        AND lower(trim(COALESCE(l.source::text, ''))) NOT IN ('web', 'inbound')
      )
    );
$$;
