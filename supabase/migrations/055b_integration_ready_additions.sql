-- Add missing integration-ready components
-- Run this if 055_integration_ready.sql fails due to existing tables
...
-- Add missing columns to integration_outbox if needed
ALTER TABLE integration_outbox 
ADD COLUMN IF NOT EXISTS idempotency_key TEXT;

-- Add unique constraint on idempotency_key if not exists
DO $$ BEGIN
  ALTER TABLE integration_outbox ADD CONSTRAINT integration_outbox_idempotency_key_key UNIQUE (idempotency_key);
EXCEPTION
  WHEN duplicate_object THEN null;
